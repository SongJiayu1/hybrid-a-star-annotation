...
namespace {
   ...
   *        查询给定节点的 configure (构型值)，分两种情况：
   *        - 若为 Node2D 类，theta分量恒为 99
   *        - 若为 Node3D 类，theta分量为 node 的真实值
   * @param node: 节点指针
   * @param x 节点 node 的 X 分量
   * @param y 节点 node 的 X 分量
   * @param t 节点 node 的 theta 分量
   */
void getConfiguration(const Node2D* node, float& x, float& y, float& t) {
  x = node->getX();
  y = node->getY();
  // avoid 2D collision checking
  t = 99; // 2D 的 getConfiguration 为多态函数，2D 网格时统一将 t=99
} 
...
/*!
   \brief The CollisionDetection class determines whether a given configuration q of the robot
    will result in a collision with the environment.
  CollisionDetection 用于检测给定机器人的一个配置 q 是否与环境发生碰撞
